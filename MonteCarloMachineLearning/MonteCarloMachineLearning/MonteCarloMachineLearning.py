...
from utils import MaxDict, PrintValues, PrintPolicy
...
def MonteCarloPolicyEvaluation(grid):
    ...
    Q = {}
    # Stores the returns for each state-action pair
    returns = {}  
    # Only consider non-terminal states
    states = grid.NonTerminalStates()  
    for s in states:
        Q[s] = {}
    ...
    for t in range(N_EPISODES):  
        ...
        StateActionPairsSeen = set()
        for s, a, G in states_actions_returns:
            sa = (s, a)
            if sa not in StateActionPairsSeen:
    ...
    V = {}
    for s in policy.keys():
        # V(s) is the maximum Q value for state s
        V[s] = MaxDict(Q[s])[1]  
  
    return V, policy, deltas

if __name__ == '__main__':
    grid = StandardGrid(obey_prob=0.8, step_cost=-0.2)
